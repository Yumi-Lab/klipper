...
class ExtruderStepper:
    ...
    def cmd_SET_PRESSURE_ADVANCE(self, gcmd):
        ...
        msg = ("ADVANCE: %.6f (configurable) — pressure advance classique : ajoute/retire de la matiere avec la vitesse ; 0 chez Yumi, LEAD_TIME fait le travail\n"
               "SMOOTH_TIME: %.6f (configurable) — lissage des rampes extrudeur (s) ; bas pour garder le pic de retract lisible par les capteurs YMS\n"
               "LEAD_TIME: %.6f (configurable) — decalage temporel de l'extrudeur (s) : il entre et sort en avance, depressurise avant la fin du segment\n"
               "BOWDEN_LENGTH: %.1f mm (configurable) — longueur du tube ; 0 = rattrapage de jeu eteint ; plafonne la courbure prise en compte\n"
               "BOWDEN_ID: %.2f mm (configurable) — diametre interieur du tube ; jeu = (ID - filament) x courbure\n"
               "BOWDEN_TURNS: %.2f (configurable) — tours de tube du cheminement (hypothese declaree) : fixe la courbure, donc le jeu\n"
               "BACKLASH_COEF: %.3f (configurable) — multiplie le jeu calcule ; 1 = modele, mesure banc 1.08\n"
               "BACKLASH_SPEED: %.1f mm/s (configurable) — vitesse de la rampe de rattrapage jouee a chaque inversion\n"
               "BACKLASH_ACCEL: %.0f mm/s2 (configurable) — acceleration de cette rampe (profil en S, pas de saut moteur)\n"
               "BACKLASH_DEDUCT: %.3f mm (configurable) — au retour, repousse le jeu MOINS cette longueur : pas de pression quand la buse arrive (anti-goutte)\n"
               "BACKLASH_BLEED: %.1f mm (configurable) — distance d'extrusion sur laquelle le residu de la deduction est rendu\n"
               "BACKLASH_RESTART: %.3f mm (configurable) — longueur en plus a chaque reprise d'extrusion : depose de la matiere, reglage live a reporter dans le trancheur\n"
               "TRAVEL_CREEP_RATE: %.4f mm/mm (configurable) — retraction continue pendant un deplacement a vide, par mm de travel ; 0 = off\n"
               "TRAVEL_CREEP_MAX: %.3f mm (configurable) — retraction cumulee maximale pendant un travel\n"
               "TRAVEL_CREEP_MIN_DIST: %.1f mm (configurable) — longueur de travel a partir de laquelle le creep demarre\n"
               "TRAVEL_CREEP_BLEED: %.1f mm (configurable) — distance d'extrusion sur laquelle le creep est rembourse ; 0 = d'un coup\n"
               "backlash_play: %.3f mm (deduced) — jeu calcule = (BOWDEN_ID - filament) x courbure x BACKLASH_COEF\n"
               "backlash_ramp: %.1f ms (deduced) — duree de la rampe de rattrapage, bornee a 100 ms"
