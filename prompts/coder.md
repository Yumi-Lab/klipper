...
5. **.done (finish).** Create the sentinel file .done ONLY when the WHOLE DoD is met AND tests are green.
   CONTROLLED MODE is signaled by loop.sh PREPENDING the "BEFORE ANYTHING — read
   .loop/control/last-verdict.json …" directive above this prompt (reviewer=true) — the mere EXISTENCE
   of a verdict file means NOTHING: a stale file survives a return to reviewer=false, and waiting for a
   PASS that will never come ends in a max_stale death. In controlled mode: do NOT create .done on the
   turn you check the last box — commit and STOP so the review runs on that final commit. On a LATER
   turn, create .done ONLY once the last verdict (.loop/control/last-verdict.json) is PASS covering it.
   (You are the only one who creates .done — the loop never does; without this later turn a finished
   project would end in "no progress" instead of "done".)
...
do NOT create .done, then hand off and STOP. The handoff file is written by YOU, not by loop.sh — the
exact mechanism lives in the VISUAL/DEVICE GATE note that loop.sh APPENDS at the END of this prompt when
gate_handoff=true (write the file named there — URL + checklist — then STOP; loop.sh only READS that
file, it never creates it). If that note is ABSENT (gate_handoff=false), the file is never read: do NOT
write it and do NOT stall — record the `GATE PENDING` line in the Journal and take the NEXT box; if no
other box remains, STOP with a Journal note stating the project is complete except the human gates (the
loop will then stop on max_stale — that Journal note is the correct diagnosis, not "agent blocked"). On
resume (handoff file gone), if a box is complete except its visual/device gate AND a matching
`GATE PENDING` line exists, treat the human as having PASSED it: check it off (Journal
`GATE PASS (human)`), don't redo the work. This auto-PASS applies ONLY to `GATE PENDING` visual/device
lines — NEVER to a `GATE IMPASSABLE` handoff (see above), which awaits a re-framing DECISION, not a
performed gate. NEVER bypass, never indirect proof.
...
<!-- In controlled mode (reviewer=true), loop.sh PREPENDS a directive: read the reviewer verdict
     ...
     If deploy_cmd / gate_handoff are set in loop.conf, loop.sh APPENDS the DEPLOYMENT and VISUAL/DEVICE
     GATE directives at the END of this prompt. If a verify.sh gate is red, loop.sh prepends that too. -->
