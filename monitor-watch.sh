...
#   HANDOFF_FILE=.gate-handoff   DONE_FILE=.done   MONITOR_DIR=.monitor (sinon clé lue du
#                           loop.conf du repo surveillé, M15)
#   CTL_DIR=.loop/control   dossier du verdict de revue (VERDICT_FILE=$CTL_DIR/last-verdict.json,
#                           miroir exact des défauts de loop.sh — surchargeable, même priorité env)
...
# Verdict de revue : mêmes défauts que loop.sh (CTL_DIR override de premier rang) — zéro hardcodage.
CTL_DIR="${CTL_DIR:-.loop/control}"
VERDICT_FILE="${VERDICT_FILE:-$CTL_DIR/last-verdict.json}"

# Intervalles : entiers ≥ 1 EXIGÉS (POLL=abc = busy-loop CPU, HEARTBEAT=0 = battement mort —
# même discipline que le préflight de loop.sh).
for _v in HEARTBEAT POLL STALL_HB; do
  eval "_n=\$$_v"
  case "$_n" in ''|*[!0-9]*) echo "❌ monitor-watch: $_v='$_n' : entier attendu." >&2; exit 1 ;; esac
  [ "$_n" -ge 1 ] || { echo "❌ monitor-watch: $_v=0 : valeur ≥ 1 exigée (0 = busy-loop / battement mort)." >&2; exit 1; }
done
...
cycles() {  # shlint-ok (branches if/return EXCLUSIVES : une seule valeur émise par appel)
  ...
  # Mode contrôlé : loop.sh écrit le MÊME marqueur dans coder-main.jsonl ET reviewer.jsonl →
  # dédoublonnage sort -u AVANT le compte, sinon chaque itération compte DOUBLE.
  n="$(cat "$MONITOR_DIR"/*.jsonl 2>/dev/null | grep '"type":"loop"' | sort -u | wc -l | tr -d ' ')"
...
while true; do
  ...
  if [ -f "$HANDOFF_FILE" ]; then
    ...
    # (VERDICT_FILE est hissé auprès de ses pairs en tête — respecte CTL_DIR/VERDICT_FILE en env.)
    _wake=1
